...
/** Transform applied to a text node's squashed output. */
export type OutputTransformer = (text: string, index: number) => string;
...
export type DOMElement = {
	...
	// Absolute layout rectangle computed during render (see
	// render-node-to-output.ts). Used by hit-testing for mouse events.
	...
	// Mouse support additions for the agav Ink fork.
...
export const emitLayoutListeners = (rootNode: DOMElement): void => {
	if (
		rootNode.nodeName !== "ink-root" ||
		!rootNode.internal_layoutListeners
	) {
		return;
	}
