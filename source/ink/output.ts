			...
			if (operation.type === "write") {
				...
				for (let [index, line] of lines.entries()) {
					...
					if (
						...
						offsetX > 0 &&
						this.caches.getStringWidth(
							currentLine[offsetX - 1]?.value ?? "",
						) > 1
					) {
					...
					for (const character of characters) {
						...
						// Determine printed width using string-width to align
						// with measurement
						...
						// For multi-column characters, clear following cells to
						// avoid stray spaces/artifacts
						if (characterWidth > 1) {
