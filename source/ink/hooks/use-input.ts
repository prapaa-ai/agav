...
type Options = {
	 ...
	 * `useInput` hooks used at once to avoid handling the same input several
	 * times.
	 *
	 * @default true
	 */
 ...
 * `data` events. The callback you pass to `useInput` is called for each
 * character when the user enters any input. However, if the user pastes text
 * and it's more than one character, the callback will be called only once, and
...
const useInput = (inputHandler: Handler, options: Options = {}): void => {
	...
	const handleData = useEffectEvent((data: Uint8Array | string) => {
		...
		if (keypress.isKittyProtocol) {
			...
			// reportAllKeysAsEscapeCodes flag is enabled), suppress
			// non-printable
			if (keypress.isPrintable) {
				...
				// the letter name must flow through so handlers (e.g.
				// exitOnCtrlC checking `input === 'c' && key.ctrl`) still work.
				input = keypress.name;
			} else {
				input = "";
			}
