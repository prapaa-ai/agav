...
4. Keep slides scannable: a short title, at most 4-6 bullets per slide, and no full paragraphs. Put the detail in speaker notes (`slide.notes_slide.notes_text_frame`) instead. Verify that `slide.notes_slide.notes_text_frame` is not `None` before writing notes; if missing, handle appropriately or report that speaker notes cannot be attached to that layout.
